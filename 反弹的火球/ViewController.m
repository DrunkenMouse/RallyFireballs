...
- (void)viewDidLoad {
    ...
    _pos = CGPointMake(15.0, 7.5);//火球每次移动的x,y
   
    //添加火球
    _fireBall = [[UIImageView alloc] initWithImage:[UIImage imageNamed:@"1"]];
    _fireBall.frame = CGRectMake(0, 0, 32, 32);
...
-(void)onTimer {
    ...
    //超出范围就取反值
    //由加变减，由减变加
    if (_fireBall.center.x > 320 || _fireBall.center.x < 0) {
        _pos.x = -_pos.x;
    }
    if (_fireBall.center.y > 460 || _fireBall.center.y < 0) {
        _pos.y = -_pos.y;
    }
